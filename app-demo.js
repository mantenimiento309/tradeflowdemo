...
const pageLoaders = {
  'mi-dashboard': loadMyDashboard, 'mis-envíos': loadMisEnvíos, 'mi-fda': loadMyFDA,
...
async function loadMyDashboard() {
  ...
  c.innerHTML = `<div class="fade-in">
    ...
    <div class="alert-banner">
      ...
      <a href="#" onclick="showPage('mis-envíos')" class="ab-action">Ver mis envíos &rarr;</a>
    </div>` : `
    ...
    <div class="my-dash-grid two">
      <div class="panel">
        <div class="panel-header"><div class="panel-title">Envíos Activos</div><a href="#" onclick="showPage('mis-envíos')" style="font-family:var(--font-mono);font-size:10px;color:var(--ink3)">Ver todos &rarr;</a></div>
        <div class="panel-body">
          ${shipments.length === 0 ? `<div style="text-align:center;padding:24px;color:var(--ink3)"><div style="font-size:13px;margin-bottom:8px">Sin envíos registrados</div><button class="btn-primary" onclick="openModal('modal-add')">Registrar primer envío</button></div>` :
            ...
            <div class="shipment-row" onclick="showPage('mis-envíos')">
  ...
  modal.innerHTML = `<div class="modal" style="max-width:720px">
    ...
    <div class="modal-body">
      <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:16px">
        ${[['Barco',s.vessel||'\u2014'],['Contenedor',s.container||'\u2014'],['Destino',(s.dest_port||'\u2014').split(',')[0]],['Estado',s.status]].map(([l,v])=>
        `<div class="info-cell"><div class="ic-label">${l}</div><div class="ic-value">${l==='Estado'?statusTag(v):v}</div></div>`).join('')}
      </div>
      <!-- PANEL ITACS FDA -->
      <div class="panel" style="margin-bottom:16px">
        <div class="panel-header">
          <div class="panel-title">Estado FDA — ITACS</div>
          <span style="font-family:var(--font-mono);font-size:10px;color:var(--ink4)">${s.entry_number}</span>
        </div>
        <div class="panel-body" id="itacs-panel-${s.id}">
          ...
          <div id="itacs-opciones-${s.id}" style="display:none;margin-top:14px;padding-top:14px;border-top:1px solid var(--rule)">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
              ...
              ].map(([label, color, msg, short]) => 
                \`<button onclick="guardarEstadoITACS(${s.id},'${short}','\${label}','\${msg}','\${color}')" 
                  ...
                  onmouseover="this.style.borderColor='var(--ink)'" onmouseout="this.style.borderColor='var(--rule)'">\${label}</button>\`
              ).join('')}
            </div>
          </div>
        </div>
      ...
      <div class="panel" style="margin-bottom:16px">
        <div class="panel-header">
          <div class="panel-title">Tracking Naviera en Tiempo Real</div>
          <button onclick="loadShipmentTracking(${s.id},this)" style="background:var(--ink);color:#fff;border:none;border-radius:var(--radius);padding:4px 12px;font-size:11px;cursor:pointer;font-family:var(--font-body)">Consultar</button>
        </div>
        <div class="panel-body" id="track-panel-${s.id}">
          <div style="font-size:12px;color:var(--ink3);font-family:var(--font-mono)">
            Contenedor: <strong>${s.container||'\u2014'}</strong> &nbsp;|&nbsp; BL: <strong>${s.bl_number||'\u2014'}</strong>
            &nbsp;&mdash;&nbsp; Presione "Consultar" para ver el estado actual.
          </div>
        </div>
      </div>` : ''}
      ${(s.fda_holds||[]).length ? `
      <div class="fda-hold-panel">
        <div class="fhp-title">FDA Hold &mdash; ${s.fda_holds.length} cargo(s) de refusal</div>
        <div class="fhp-sub">90 dias para responder, reconditioner o reexportar el producto.</div>
        <table class="data-table" style="background:var(--white);border-radius:var(--radius)">
          ...
          <tbody>${s.fda_holds.map(h=>`<tr>
            <td>${tag(h.charge_code,'red')}</td>
            <td style="font-family:var(--font-mono);font-size:10px;color:var(--red)">${h.section}</td>
            <td style="font-size:12px">${h.description}</td>
            <td><a href="https://www.ecfr.gov/current/title-21" target="_blank" style="font-family:var(--font-mono);font-size:10px">eCFR</a></td>
          </tr>`).join('')}</tbody>
        </table>
      </div>` : `<div class="ok-banner"><strong>Sin cargos FDA.</strong> Producto liberado sin restricciones.</div>`}
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div class="panel">
          <div class="panel-header"><div class="panel-title">Costos de Importacion</div></div>
          <table class="data-table">
            ${(s.costs||[]).filter(c=>c.type==='normal').map(c=>`<tr><td>${c.item}</td><td style="text-align:right;font-family:var(--font-mono);font-size:11px">${c.amount===0?'$0 (CAFTA)':'$'+fmt(c.amount)}</td></tr>`).join('')}
            <tr style="border-top:2px solid var(--rule)"><td style="font-weight:700">Subtotal</td><td style="text-align:right;font-weight:700;font-family:var(--font-mono)">$${fmt(total)}</td></tr>
          </table>
        </div>
        ${extra>0 ? `
        <div class="panel">
          ...
          <table class="data-table">
            ${(s.costs||[]).filter(c=>c.type==='extra').map(c=>`<tr><td>${c.item}</td><td style="text-align:right;font-family:var(--font-mono);font-size:11px;color:var(--red)">${fmt(c.amount)}</td></tr>`).join('')}
            <tr style="border-top:2px solid var(--red-border)"><td style="font-weight:700;color:var(--red)">Total con hold</td><td style="text-align:right;font-weight:700;font-family:var(--font-mono);color:var(--red)">$${fmt(total+extra)}</td></tr>
          </table>
        </div>` : `<div class="panel" style="display:flex;align-items:center;justify-content:center;flex-direction:column;gap:6px;padding:24px"><div style="font-family:var(--font-display);font-size:14px;font-weight:700;color:var(--green)">Sin costos adicionales</div></div>`}
      </div>
    </div>
  </div>`;
...
async function deleteShipment(id) {
  ...
  if (data.ok) { toast('Envío eliminado.'); loadMisEnvíos(); loadMyDashboard(); }
...
async function submitShipment(e) {
  ...
  if (data.ok) { closeModal('modal-add'); toast('Envío registrado.'); e.target.reset(); showPage('mis-envíos'); loadMyDashboard(); }
  else toast(data.msg||'Error.','err');
}
